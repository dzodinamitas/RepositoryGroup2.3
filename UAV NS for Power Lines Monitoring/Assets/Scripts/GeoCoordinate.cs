...
public sealed class GeoCoordinate
{
	...
	public double X
	{
		...
		set
		{
			Assert.IsTrue((_x + _xOffset) <= 20000000 && (_x + _xOffset) >= -20000000);
			_x = value;
	...
	public double Y
	{
		...
		set
		{
			Assert.IsTrue((_y + _yOffset) <= 10000000 && (_y + _yOffset) >= -10000000);
			_y = value;
	...
	{
		Assert.IsTrue(xOffset <= 20000000 && xOffset >= -20000000);
		Assert.IsTrue(yOffset <= 10000000 && yOffset >= -10000000);
		_xOffset = xOffset;
		_yOffset = yOffset;
	}
